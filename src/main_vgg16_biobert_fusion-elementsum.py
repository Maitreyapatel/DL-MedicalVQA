...
class EarlyStopping:
    ...
    def __init__(self,
                 ...
                 verbose=False,
                 delta=0,
                 path='./models/vgg16_fusion-elementsum/checkpoint.pt',
                 trace_func=print):
        """
        Args:
...
writer = SummaryWriter('runs/vgg16_fusion-elementsum_run_1')
...
trainer.model.load_state_dict(torch.load('./models/vgg16_fusion-elementsum/checkpoint.pt'))
trainer.model.eval()
...
pd.DataFrame({'imageids':imageids, 'answers':answers}).to_csv('vgg16_fusion-elementsum.txt', sep='|', index=False, header=False)
